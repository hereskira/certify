...
class CertifyGUI(QWidget):
    def __init__(self):
        ...
        self.setGeometry(300, 100, 600, 820)
    ...
    def add_signatory(self):
        if len(self.signatories) >= 3:
            QMessageBox.warning(self, "Limit reached", "You can only add up to 3 signatories.")
            return
        ...
        def upload():
            path, _ = QFileDialog.getOpenFileName(self, "Select Signature", "", "Image Files (*.png *.jpg *.jpeg)")
            if path:
                sig_data["signature_path"] = path
                preview.setPixmap(QPixmap(path).scaledToWidth(100))
    ...
    def create_event(self):
        ...
        self.output_log.append(f"Created event: {title}")
    ...
    def generate_certificates(self):
        ...
        if not os.path.exists(participants_csv):
            QMessageBox.warning(self, "Error", "Participants CSV not found.")
            return
